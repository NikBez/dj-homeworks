...
def test_filter_courses_by_id(client, courses_factory):
    ...
    response = client.get('/api/v1/courses/?id=100')
    assert response.status_code == 200
    data = response.json()
...
def test_update_course(client, student_factory, courses_factory):
    courses_factory(_quantity=10, id=1)
    students = student_factory(_quantity=10)
    ...
    response = client.patch('/api/v1/courses/1/', data=data, format='json')
...
@pytest.mark.django_db
def test_max_students_course(client, student_factory, max, settings):
